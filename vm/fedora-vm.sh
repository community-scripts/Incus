...
# ==============================================================================
# Fedora VM on an Incus host.
#
# Fedora ships a new release roughly every six months and each one is supported
# for about thirteen months, so the version pinned here is a moving target in a
# way Debian's is not. Override var_version and var_image together.
#
# The guest is sized above the Debian default on purpose: dnf resolves
# dependencies in memory and a stock Fedora root filesystem is noticeably
# larger than a stock Debian one.
# ==============================================================================

APP="Fedora"
APP_TYPE="vm"
var_os="${var_os:-fedora}"
var_version="${var_version:-44}"
var_cpu="${var_cpu:-2}"
var_ram="${var_ram:-4096}"
var_disk="${var_disk:-16G}"
# Plain images:fedora/44 has no cloud-init. incus_vm_create switches to the
# /cloud variant automatically when cloud-init is enabled.
var_image="${var_image:-images:fedora/44}"

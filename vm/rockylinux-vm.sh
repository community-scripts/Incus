...
# ==============================================================================
# Rocky Linux VM on an Incus host.
#
# Deliberately not a copy of ProxmoxVE's vm/rockylinux-vm.sh: that one downloads
# a cloud image and imports it with qm/pvesm. Incus launches a VM image straight
...
# Plain images:rockylinux/10 has no cloud-init. incus_vm_create switches to the
# /cloud variant automatically when cloud-init is enabled.
# amd64 only: the images: remote ships no aarch64 VM build of Rocky Linux.
var_image="${var_image:-images:rockylinux/10}"

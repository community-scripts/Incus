...
# Source: https://en.altlinux.org/

# ==============================================================================
# ALT Linux VM on an Incus host.
#
# ALT is RPM-based but driven by apt-rpm, so the package tooling is apt-get /
# apt-repo rather than dnf; nothing in this script assumes otherwise. p11 is the
# current stable branch and is the alias published on the images: remote.
#
# There is no ProxmoxVE counterpart to copy from: Incus launches a VM image
# straight from a remote and configures it through native keys.
# ==============================================================================

APP="ALT Linux"
APP_TYPE="vm"
var_os="${var_os:-alt}"
var_version="${var_version:-p11}"
var_cpu="${var_cpu:-2}"
var_ram="${var_ram:-2048}"
var_disk="${var_disk:-12G}"
# Plain images:alt/p11 has no cloud-init. incus_vm_create switches to the
# /cloud variant automatically when cloud-init is enabled.
var_image="${var_image:-images:alt/p11}"

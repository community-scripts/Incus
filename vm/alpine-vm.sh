...
# ==============================================================================
# Alpine Linux VM on an Incus host.
#
# Alpine is musl + busybox and boots a full VM in a few hundred MiB, so the
# defaults below are deliberately smaller than every other VM script here.
# Raise them in the advanced flow if the workload actually needs it.
#
# The guest has no bash: the shell hint at the end uses sh, which busybox
# provides. Alpine also ships OpenRC rather than systemd, so anything copied
# from a Debian guide that calls systemctl will not work inside this VM.
# ==============================================================================
...
var_cpu="${var_cpu:-1}"
var_ram="${var_ram:-512}"
var_disk="${var_disk:-4G}"
# Plain images:alpine/3.22 has no cloud-init. incus_vm_create switches to the
# /cloud variant automatically when cloud-init is enabled.
var_image="${var_image:-images:alpine/3.22}"

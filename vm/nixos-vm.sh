...
# ==============================================================================
# NixOS VM on an Incus host.
#
# NixOS is declarative: the guest is described by configuration.nix and built
# with nixos-rebuild, not assembled by an install script running commands inside
# it. That is the opposite of how the rest of this collection provisions a
# container, so a VM is the honest fit - hand over a booted machine and let the
# configuration own everything above it.
#
# Sized for that model: a nixos-rebuild evaluates and builds inside the guest,
# and the Nix store keeps every generation until it is garbage collected, so
# both RAM and disk are well above what a running NixOS actually consumes.
# ==============================================================================

APP="NixOS"
APP_TYPE="vm"
var_os="${var_os:-nixos}"
var_version="${var_version:-unstable}"
var_cpu="${var_cpu:-2}"
var_ram="${var_ram:-4096}"
var_disk="${var_disk:-20G}"
# Plain images:nixos/unstable has no cloud-init. incus_vm_create switches to the
# /cloud variant automatically when cloud-init is enabled.
var_image="${var_image:-images:nixos/unstable}"

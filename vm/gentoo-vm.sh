...
# ==============================================================================
# Gentoo VM on an Incus host.
#
# Gentoo builds packages from source, so the defaults below are the largest of
# the VM scripts here: cores because emerge parallelises across them, disk
# because /var/tmp/portage plus a synced ::gentoo tree dwarfs the installed
# system. A single-core Gentoo VM is technically valid and practically useless.
#
# Rolling release, so there is no version number to pin - the alias picks the
# init system instead, and images:gentoo/systemd is the systemd stage rather
# than the OpenRC default.
# ==============================================================================

APP="Gentoo"
APP_TYPE="vm"
var_os="${var_os:-gentoo}"
var_version="${var_version:-systemd}"
var_cpu="${var_cpu:-6}"
var_ram="${var_ram:-8192}"
var_disk="${var_disk:-40G}"
# amd64 only: images: publishes no aarch64 VM build of Gentoo, so this script
# cannot run on an ARM Incus host.
# Plain images:gentoo/systemd has no cloud-init. incus_vm_create switches to the
# /cloud variant automatically when cloud-init is enabled.
var_image="${var_image:-images:gentoo/systemd}"

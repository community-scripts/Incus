...
# ==============================================================================
# Arch Linux VM on an Incus host.
#
# Rolling release, so the image alias carries no version and the guest is only
# as current as the day it was published: run `pacman -Syu` before installing
# anything, and never a partial upgrade. The disk default leaves room for the
# pacman package cache under /var/cache/pacman/pkg, which is never pruned
# automatically.
# ==============================================================================
...
var_version="${var_version:-rolling}"
var_cpu="${var_cpu:-2}"
var_ram="${var_ram:-2048}"
var_disk="${var_disk:-16G}"
# amd64 only: the images: remote publishes no aarch64 VM build for Arch, so
# this script cannot run on an ARM host.
# Plain images:archlinux has no cloud-init. incus_vm_create switches to the
# /cloud variant automatically when cloud-init is enabled.
var_image="${var_image:-images:archlinux}"

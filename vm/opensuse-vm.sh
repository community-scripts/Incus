...
# ==============================================================================
# openSUSE Tumbleweed VM on an Incus host.
#
# Tumbleweed is rolling, so there is no release number to pin: the image alias
# always points at the current snapshot and the first `zypper dup` after
# creation can be large. The disk default allows for that, and for the btrfs
# root snapshots the image ships with -- those are what actually fill a small
# openSUSE disk, not the installed packages.
# ==============================================================================

APP="openSUSE"
APP_TYPE="vm"
var_os="${var_os:-opensuse}"
var_version="${var_version:-tumbleweed}"
var_cpu="${var_cpu:-2}"
var_ram="${var_ram:-4096}"
var_disk="${var_disk:-20G}"
# Plain images:opensuse/tumbleweed has no cloud-init. incus_vm_create switches
# to the /cloud variant automatically when cloud-init is enabled.
var_image="${var_image:-images:opensuse/tumbleweed}"

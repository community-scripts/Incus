...
# ==============================================================================
# FreeBSD VM on an Incus host.
#
# The only distribution here that exists as a VM image and nothing else: an
# Incus container shares the host's Linux kernel, and a FreeBSD userland cannot
# run on one. There is no container path to fall back to, so this is VM-only by
# nature rather than by preference.
#
# GUEST_OS defaults to FreeBSD, which is not cosmetic - Incus disables memory
# hotplug for FreeBSD guests because the kernel does not handle it.
# ==============================================================================

APP="FreeBSD"
APP_TYPE="vm"
var_os="${var_os:-freebsd}"
var_version="${var_version:-15.1}"
var_cpu="${var_cpu:-2}"
var_ram="${var_ram:-2048}"
var_disk="${var_disk:-12G}"
# Plain images:freebsd/15.1 has no cloud-init. incus_vm_create switches to the
# /cloud variant automatically when cloud-init is enabled.
var_image="${var_image:-images:freebsd/15.1}"

...
# ==============================================================================
# Umbrel OS VM on an Incus host.
#
# The other VM scripts in this folder launch an alias from the images: remote.
# There is none for Umbrel OS: it is published only as a vendor disk image, so
# this downloads one and hands the file to incus_vm_create, which imports it as
# an Incus image before creating the instance.
#
# The download is a raw image, so incus_vm_create converts it to qcow2 before
# importing - that conversion needs roughly the image size again in TMPDIR, on
# top of the download cache.
#
# Umbrel OS is an appliance and ships no incus-agent. `incus exec` will not work
# and `incus list` shows no address - use the console, or look the lease up on
# your DHCP server.
# ==============================================================================
...
APP_TYPE="vm"
NSAPP="umbrel-os-vm"
var_os="${var_os:-umbrel-os}"
# The vendor publishes a rolling "latest" build with no version in the path.
var_version="${var_version:-latest}"
var_cpu="${var_cpu:-2}"
...
case "$(uname -m)" in
x86_64 | amd64) ;;
*) fatal "Umbrel OS is published for amd64 only - this host is $(uname -m)" ;;
esac

function default_settings() {
  HN="${var_hostname:-umbrelos}"
  CORE_COUNT="$var_cpu"
  RAM_SIZE="$var_ram"
  DISK_SIZE="$var_disk"
  BRG="$var_bridge"
...
if [[ -n "${VM_IP:-}" ]]; then
  ...
  echo -e "${TAB}${GATEWAY}${BGN}Web UI:  ${CL}http://${VM_IP}"
else
  echo -e "${TAB}${GATEWAY}${BGN}Address: ${CL}not known yet - run: incus list ${HN}"
  echo -e "${TAB}${GATEWAY}${BGN}Web UI:  ${CL}port 80 on that address"
fi
